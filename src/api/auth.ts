    ...
    if (req.cookies.token) {
        const token_done_func = (usr: bsyr_user_resp | null, err: string | null) => {
            if (usr) {
                ...
                send_status_resp(401, "User not logged in", res);
            }
        };
        ...
        if (result) {
            const on_hash_comp_resolved = (match: boolean) => {
                if (match) {
                    ...
                    send_status_resp(200, "Incorrect password", res);
                }
            };
            const on_hash_comp_rejected = (reason: any) => {
                send_status_resp(500, reason, res);
            };
            ...
            send_status_resp(200, "User not found", res);
        }
    };
...
export function create_auth_routes(mongo_client: MongoClient): Router {
    ...
    const login = (req: Request, res: Response) => {
        ...
        if (!email || !plain_text_pwd) {
            send_status_resp(200, "Username and password are required", res);
            return;
        }
